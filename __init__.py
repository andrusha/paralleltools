...
def filter(function, iterable, threads = 5, result_callback = None):
    return _start_worker(FilterWorker, function, iterable, threads, result_callback, async = False)

def map(function, iterable, threads = 5, result_callback = None):
    return _start_worker(MapWorker, function, iterable, threads, result_callback, async = False)

def async_filter(function, iterable, threads = 5, callback = None):
    return _start_worker(FilterWorker, function, iterable, threads, callback, async = True)

def async_map(function, iterable, threads = 5, callback = None):
    return _start_worker(MapWorker, function, iterable, threads, callback, async = True)
